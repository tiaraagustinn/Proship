...
export default function JadwalPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert('Gagal menambah jadwal: ' + (result.message || result.error || 'Unknown error'));
        return;
      }
      ...
      alert('Gagal menambah jadwal: ' + String(err));
    }
  };
      ...
      <div className="rounded-xl overflow-hidden border-8 border-[#8ba1fa] shadow-lg">
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center text-red-300 bg-gray-600">{error}</div>
          ) : (
